...
def main():
    ...
    if len(sys.argv) != 4:
        ...
        exit()
    ROOT_PATH, strategy_build_context, context_threshold= sys.argv[1], sys.argv[2], float(sys.argv[3])
    length_text_context = int(12_000 * (1.0 - context_threshold))
    ...
    qa_file_path = f"{ROOT_PATH}/qa-pairs/qa-pairs.json"
    folder_name = os.path.basename(os.path.normpath(f"{ROOT_PATH}"))
    output_path = f"{OUTPUT_DIR}/{folder_name}-{strategy_build_context}-{context_threshold}.json"
    ...
    valid_pairs = [entry for entry in data if
                   isinstance(entry['question'], str) and isinstance(entry['answer'], str)][:500]

    max_concurrent_tasks = 10  # Adjust based on your system's capacity
